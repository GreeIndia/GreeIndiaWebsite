...
const AdminProductView = () => {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] font-inter text-gray-900">
            {/* Header */}
            <header className="bg-white border-b border-gray-100 px-4 md:px-10 py-4 md:py-6 sticky top-0 z-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4 md:gap-6">
                    <button 
                        onClick={() => navigate('/admin')}
                        className="w-10 h-10 flex items-center justify-center bg-gray-50 hover:bg-blue-50 text-gray-400 hover:text-blue-700 rounded-xl transition-colors border border-gray-100 flex-shrink-0"
                    >
                        <ArrowLeft size={18} />
                    </button>
                    <div>
                        <h1 className="text-lg md:text-xl font-black tracking-tight leading-tight">{nameDisplay}</h1>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1 hidden md:block">Asset ID: {product._id}</p>
                    </div>
                </div>
                <div className="flex flex-wrap items-center gap-2 md:gap-3 ml-14 md:ml-0">
                    <span className={`px-3 md:px-4 py-1.5 md:py-2 rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-widest border ${product.quantity > 0 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-500 border-red-100'}`}>
                        {product.quantity > 0 ? 'In Stock' : 'Depleted'}
                    </span>
                    <span className="px-3 md:px-4 py-1.5 md:py-2 rounded-full bg-blue-50 text-blue-700 border border-blue-100 text-[9px] md:text-[10px] font-bold uppercase tracking-widest">
                        {product.variant || 'Standard Configuration'}
                    </span>
                </div>
            </header>

            <main className="p-4 md:p-10 lg:p-14 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
                {/* Left Column: Visuals & Core Specs */}
                <div className="lg:col-span-5 space-y-10">
                    <div className="bg-gradient-to-br from-gray-100 to-gray-200 rounded-[2rem] h-[300px] md:h-[500px] flex items-center justify-center p-6 md:p-12 relative overflow-hidden group">
                    ...
                    <div className="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-gray-100">
                        ...
                        <div className="grid grid-cols-2 gap-3 md:gap-4">
                            <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                <span className="block text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Base MRP</span>
                                <span className="text-lg md:text-2xl font-black text-gray-900">₹{product.mrp ? Math.round(product.mrp * 1.30).toLocaleString() : 'TBD'}</span>
                            </div>
                            <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                <span className="block text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Available Stock</span>
                                <span className={`text-lg md:text-2xl font-black ${product.quantity > 0 ? 'text-emerald-500' : 'text-red-500'}`}>{product.quantity || 0}</span>
                            </div>
                        </div>
                    </div>
                </div>
